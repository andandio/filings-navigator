...
const DashboardTable = () => {
  ...
  const buttons:ReactElement[] = [
    <Button variant="text" onClick={() => queryApi("filers")} key="filers">FILERS</Button>,
    <Button variant="text" onClick={() => queryApi("filings")} key="filings">FILINGS</Button>,
    <Button variant="text" onClick={() => queryApi("awards")} key="awards">AWARDS</Button>
  ];

  const instructions = () => {
    if (resource ==="filers") { return  "Click a row to explore a filer" }
    if (resource === "filings") { return "Click a row to filter filings by filer" }
    return ""
  }
            ...
            {displayData.map((row, i) => (
              ...
              >
                {Object.keys(row).map((key) => {
                  ...
                  return (
                    <TableCell key={`${key}-cell`} component="td" scope="row" sx={styles.tableData}>
                      <span>
                        {formatCell(row[key])}
                      </span>
                    </TableCell>
