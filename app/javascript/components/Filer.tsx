...
import { formatCell, formatHeaderString } from "../utils";
...
const Filer = () => {
  ...
  const queryAwardsByYear = (taxYear: string) => {
    ...
    axios
      .get(queryString, { params: { year: taxYear, filer_id: filer.id } })
      .then(function (response) {
  ...
  const buttons: ReactElement[] = filer?.filing_years?.map((year) => {
    ...
    return (
      <Button variant="text" onClick={() => queryAwardsByYear(year)} key={year}>
